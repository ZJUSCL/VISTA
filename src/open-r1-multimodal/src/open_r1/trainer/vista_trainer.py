...
def select_sample_indices(total_rewards):
    """
    输入: total_rewards (Tensor shape [8]), values in {0, 1, 2}
    输出: (neg_idx, pos_idx) 两个整数索引
    规则:
        - 正类: value == 2
        - 负类: value != 2
        - 如果两类都存在: 第一个返回负类索引, 第二个返回正类索引
        - 如果只存在一类: 两个都从 0-7 中完全随机返回
    """
    # 1. 获取正类 (value == 2) 和 负类 (value != 2) 的索引位置
    pos_indices = torch.where(total_rewards == 2)[0]
    neg_indices = torch.where(total_rewards != 2)[0]

    # 2. 检查边界情况：如果全部是正类 或者 全部是负类
    if len(pos_indices) == 0 or len(neg_indices) == 0:
        # 两个索引都在 0-7 之间完全随机选择
        idx1 = torch.randint(0, 8, (1,)).item()
        idx2 = torch.randint(0, 8, (1,)).item()
        # print(149, "yes")
        return idx1, idx2

    # 3. 正常情况：分别随机选择
    # 从负类索引中随机选一个
    idx1 = neg_indices[torch.randint(len(neg_indices), (1,))].item()
    # 从正类索引中随机选一个
    idx2 = pos_indices[torch.randint(len(pos_indices), (1,))].item()

    return idx1, idx2

class VISTATrainer(Trainer):
    """
    Trainer for the VISTA training pipeline.

    Example:

    ```python
    from datasets import load_dataset
    from open_r1.trainer import VISTATrainer

    dataset = load_dataset("trl-lib/tldr", split="train")

    trainer = VISTATrainer(
    ...
    Args:
        ...
        args ([`VISTAConfig`], *optional*, defaults to `None`):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if args is None:
            ...
            args = VISTAConfig(f"{model_name}-VISTA")
        ...
        if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
            ...
            raise ValueError(
                "Invalid `torch_dtype` passed to `VISTAConfig`. Expected either 'auto' or a string representing "
        ...
        # Data collator
        def data_collator(features):  # No data collation is needed in VISTA
            return features
        ...
        self.max_completion_length = args.max_completion_length  # = |o_i| in the VISTA objective
        self.num_generations = args.num_generations  # = G in the VISTA objective
        ...
        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the VISTA objective
        ...
        # input tensor associated with the key "input_ids". However, in VISTA, the sampled data does not include the
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: VISTAConfig) -> PreTrainedModel:
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In VISTATrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        solutions = inputs[0]["solution"]
        images = inputs[0]["image"]
        sw_list = inputs[0]["width_resized"]
        sh_list = inputs[0]["height_resized"]

        # print(nums)
        for i in range(len(images)):
            w, h = images[i].size
            # solutions[i] = [int(solutions[i][0] * sw),int(solutions[i][1] * sh),int(solutions[i][2] * sw),int(solutions[i][3] * sh)]
            inputs[i]['solution'] = solutions[i]
                        # 'width_resized': sw,
            # 'height_resized': sh,
            inputs[i]['width_resized'] = sw_list[i]
            inputs[i]['height_resized'] = sh_list[i]
        ...
        # print('\n\nyy1', prompt_ids.shape, prompt_mask.shape, prompt_inputs['pixel_values'])
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            forbid = [
                unwrapped_model.config.image_token_id,
                unwrapped_model.config.video_token_id,
                unwrapped_model.config.vision_start_token_id,
                unwrapped_model.config.vision_end_token_id,
            ]
            bad_words_ids = [[t] for t in forbid if t is not None]
            # for k, v in prompt_inputs.items():
            #     if k not in self.vlm_module.get_non_generate_params():
            #         print('yy22:', k, v.shape)
        ...
        print(601, completions)
        if is_conversational(inputs[0]):
            completions = [[{"role": "assistant", "content": completion}] for completion in completions]
        # print(602, prompts)
        # print(603, self.processing_class.apply_chat_template(prompts[0], tokenize=False, add_generation_prompt=True))
        # assert 0
        # Compute the rewards
        ...
        self.num_manual_generations = 1
        new_prompts = []
        new_completions = []
        new_solutions = []
        new_images = []
        for i in range(len(prompts)):
            new_prompts.append(self.processing_class.apply_chat_template(prompts[i], tokenize=False, add_generation_prompt=True))

            completion = f"{completions[i][0]['content']}<|im_end|>"
            ...
            if (i + 1) % self.num_generations == 0:
                total_rewards = rewards_per_func.sum(dim=1)
                idx1, idx2 = select_sample_indices(total_rewards)

                new_prompts.append(self.processing_class.apply_chat_template(prompts[idx1], tokenize=False, add_generation_prompt=True))
                x = int((solutions[idx1][0] + solutions[idx1][2])/2)
                y = int((solutions[idx1][1] + solutions[idx1][3])/2)
                completion = f"[{x}, {y}]<|im_end|>"
                new_completions.append(completion)

                new_solutions.append(solutions[idx1])

                new_images.append(images[idx1])

                new_reward = torch.tensor([[rewards_per_func[idx2,0], rewards_per_func[idx2,1]]], device=device)
                rewards_per_func = torch.cat((rewards_per_func, new_reward), dim=0)
        ...
        gathered_reward_func = [None] * world_size
        dist.all_gather_object(gathered_reward_func, rewards_per_func)
        gathered_reward_func = [
                                    row.tolist()
                                    for reward_tensor in gathered_reward_func
                                    for row in reward_tensor
                                ]
        dist.barrier()
        cols = [f.__name__ for f in self.reward_funcs]

        df = pd.DataFrame(gathered_reward_func, columns=cols)
        df["total_score"] = df[cols[0]] * df[cols[1]]
        block_size = self.num_generations + self.num_manual_generations

        is_model_generation = (df.index % block_size) < self.num_generations
        df_gen_only = df[is_model_generation]
        content_scores = df_gen_only.groupby(df_gen_only.index // block_size)["total_score"]

        all_0_or_1_ratio = content_scores.apply(lambda x: x.isin([0]).all()).mean()
        all_2_ratio = content_scores.apply(lambda x: x.eq(1).all()).mean()
        self._metrics["ratio/all_0"].append(all_0_or_1_ratio)
        self._metrics["ratio/all_2"].append(all_2_ratio)
        content_scores = df.groupby(df.index % block_size)["total_score"]
        group_mean = content_scores.mean()
        for i in range(len(group_mean)):
            self._metrics[f"rewards/avg_{i}"].append(group_mean[i])

        rewards_per_func = self.accelerator.gather(rewards_per_func)
        gpu_id = torch.cuda.current_device()
        rewards = rewards_per_func.sum(dim=1)
        if gpu_id == 0:
            print(874, rewards.view(-1, block_size)[:, -1])
        # Compute grouped-wise rewards
        # Each group consists of num_generations completions for the same prompt
        mean_grouped_rewards = rewards.view(-1, block_size)[:, :self.num_generations].mean(dim=1)
        std_grouped_rewards = rewards.view(-1, block_size)[:, :self.num_generations].std(dim=1)
        # Normalize the rewards to compute the advantages
        mean_grouped_rewards = mean_grouped_rewards.repeat_interleave(block_size, dim=0)
        std_grouped_rewards = std_grouped_rewards.repeat_interleave(block_size, dim=0)
        advantages = (rewards - mean_grouped_rewards) / (std_grouped_rewards + 1e-4)

        # Get only the local slice of advantages
        process_slice = slice(
            self.accelerator.process_index * block_size,
            (self.accelerator.process_index + 1) * block_size,
        )
        ...
        reward_per_func = self.accelerator.gather_for_metrics(rewards_per_func[:, :self.num_generations]).mean(0)
        for i, reward_func in enumerate(self.reward_funcs):
            if isinstance(reward_func, PreTrainedModel):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The VISTATrainer does not support returning outputs")
        ...
        if self.beta > 0:
            ...
            per_token_kl = torch.clamp(per_token_kl, -50, 50)

            x = (ref_per_token_logps - per_token_logps).float()      
            x = x.clamp(-20, 20)                                     
            per_token_kl = torch.expm1(x) - x                        
            per_token_kl = per_token_kl.clamp(-50, 50)               

            per_token_loss = per_token_loss + self.beta * per_token_kl

            # Log KL divergence
            den = completion_mask.sum(dim=1)
            num = (per_token_kl * completion_mask).sum(dim=1)
            mean_kl = num / den.clamp_min(1)     
    ...
    def _get_train_sampler(self, train_dataset=None) -> Sampler:
        """Returns a sampler that ensures proper data sampling for VISTA training."""
